...
def load_data():
    file = open("data/restaurant/doc2vec/X_train_doc2vec_rest",'rb')
    X_train_doc2vec_rest = pickle.load(file)
    file.close()

    file = open("data/restaurant/doc2vec/y_train_doc2vec_rest",'rb')
    y_train_doc2vec_rest = pickle.load(file)
    file.close()

    file = open("data/restaurant/doc2vec/X_test_doc2vec_rest",'rb')
    X_test_doc2vec_rest = pickle.load(file)
    file.close()

    file = open("data/restaurant/doc2vec/y_test_doc2vec_rest",'rb')
...
def main():
    for i in range(5):
        ...
        with open('model/aspect_extraction/feature_regression_mlp_doc2vec_{}.p'.format(i), 'wb') as fp:
            pickle.dump(mlp, fp)
        print('model is save to model/aspect_extraction/feature_regression_mlp_doc2vec_{}.p'.format(i))
