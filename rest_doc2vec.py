...
def import_model():
    model = g.Doc2Vec.load("model/enwiki_dbow/doc2vec.bin")
    return model
...
def doc2vec(X_train, X_test, y_train, y_test):
    ...
    with open('data/restaurant/doc2vec/X_train_doc2vec_rest', 'wb') as fp:
        pickle.dump(X_train_vector, fp)
    with open('data/restaurant/doc2vec/X_test_doc2vec_rest', 'wb') as fp:
        pickle.dump(X_test_vector, fp)
    with open('data/restaurant/doc2vec/y_train_doc2vec_rest', 'wb') as fp:
        pickle.dump(y_train, fp)
    with open('data/restaurant/doc2vec/y_test_doc2vec_rest', 'wb') as fp:
