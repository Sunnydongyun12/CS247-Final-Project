...
def bow(X_train, X_test, y_train, y_test):
    ...
    for line in X_test:
        ...
        for w in words:
            w = stemmer.stem(w)
            temp.append(w)
        X_test_stem.append(" ".join(temp))
    print('stemming success!!!')

    vectorizer = import_model()
    ...
    X_train_bow_rest = tfidf_transformer.fit_transform(training_vectors)
    X_test_bow_rest = tfidf_transformer.transform(testing_vectors)
    print('the final tfidf train matrix with shape {}'.format(X_train_bow_rest.shape))
    print('the final tfidf test matrix with shape {}'.format(X_test_bow_rest.shape))
    print('yeah! done!')

    with open('data/restaurant/bow/X_train_bow_rest', 'wb') as fp:
        pickle.dump(X_train_bow_rest, fp)
    with open('data/restaurant/bow/X_test_bow_rest', 'wb') as fp:
        pickle.dump(X_test_bow_rest, fp)
    with open('data/restaurant/bow/y_train_bow_rest', 'wb') as fp:
        pickle.dump(y_train, fp)
    with open('data/restaurant/bow/y_test_bow_rest', 'wb') as fp:
