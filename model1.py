...
def import_model(i):
    file = open("model/aspect_extraction/feature_regression_mlp_doc2vec_{}.p".format(i),'rb')
    aspect_mlp = pickle.load(file)
    return aspect_mlp

def load_data():
    with open('data/yelp/doc2vec/yelp_doc2vec_reviews.p', 'rb') as f:
        train_data = pickle.load(f)
    with open('data/yelp/doc2vec/yelp_doc2vec_labels.p', 'rb') as f:
